...
export class ViewModule {
    ...
    createEffect(type) {
        ...
        if (type === 'conveyor') {
            ...
            this.registerTooltip(display, () => {
                var _a;
                return (_a = display.tooltip) !== null && _a !== void 0 ? _a : '';
            });
            display.on('mouseover', () => { var _a, _b; return (_b = (_a = display).mouseOver) === null || _b === void 0 ? void 0 : _b.call(_a); });
            display.on('mouseout', () => { var _a, _b; return (_b = (_a = display).mouseOut) === null || _b === void 0 ? void 0 : _b.call(_a); });
            display.anchor.x = 0;
    ...
    updateScene(previousData, currentData, progress, playerSpeed) {
        ...
        for (const [index, hex] of Object.entries(this.hexes)) {
            if (hex.icon != null) {
                hex.iconBounceContainer.zIndex = 1;
            }
    ...
    resetParticles() {
        ...
        for (let playerIdx = 0; playerIdx < this.globalData.playerCount; ++playerIdx) {
            ...
            const particleGroupByTile = {};
            const particleGroupByMoveTo = {};
            for (const { index } of this.globalData.cells) {
                ...
                if (stayingAnts > 0) {
                    ...
                    const group = { particles, cellIdx: index, playerIdx };
            ...
            if (lastShownData === this.previousData && this.particleGroupsByPlayer.length > 0) {
                ...
                for (let playerIdx = 0; playerIdx < this.globalData.playerCount; ++playerIdx) {
                    ...
                    for (const { index } of this.globalData.cells) {
                        ...
                        const availableParticles = ((_a = currentGroupStayingOnTile === null || currentGroupStayingOnTile === void 0 ? void 0 : currentGroupStayingOnTile.particles) !== null && _a !== void 0 ? _a : [])
                            .concat((_b = currentGroupMovedOntoTile === null || currentGroupMovedOntoTile === void 0 ? void 0 : currentGroupMovedOntoTile.flatMap(group => group.particles)) !== null && _b !== void 0 ? _b : []);
                    ...
                    Object.values(availableParticlesByCellIdx).flatMap(arr => arr).forEach(particle => {
    ...
    drawParticleGroup(amount, playerIdx) {
        const particles = [];
    ...
    updateMoves() {
        ...
        for (const event of this.currentData.events.filter(({ type }) => type === ev.MOVE)) {
            ...
            const { display } = this.getFromPool('arrow');
            const sprite = display.children[0].children[0];
            const number = display.children[1];
    ...
    updateGains() {
        ...
        for (const event of this.currentData.syntheticEvents) {
            ...
            for (const total of event.totals) {
                ...
                const playerIdx = event.playerIdx;
                const g = this.getFromPool('antText');
                ...
                const targetP = Object.assign(Object.assign({}, pos), { y: pos.y - 100 / this.boardOverlay.scale.x });
            ...
            for (const segment of event.segments) {
                const { display } = this.getFromPool('conveyor');
            ...
            for (const segment of event.segments) {
                ...
                conveyor.tooltip = `Earned ${segment.amount} ${event.type === ev.FOOD ? 'point' : 'ant'}${segment.amount !== 1 ? 's' : ''}`;
            }
        }
    }
    screenToBoard(point) {
    ...
    getOwnersOfBeaconOn(cellIdx, data) {
    ...
    animateAntParticles(fromIdx, toIdx, particles, progress) {
        const speed = 2;
    ...
    initHexData(cell) {
        ...
        let foodTextBackground = null;
        let foodText = null;
        let icon = null;
        let iconBounceContainer = null;
        if (cell.richness > 0) {
    ...
    handleGlobalData(players, raw) {
        ...
        const maxScore = globalData.cells
            ...
            .reduce((a, b) => a + b, 0);
        this.globalData = Object.assign(Object.assign({}, globalData), { players: players, playerCount: players.length, anthills,
            maxScore });
    ...
    createExplosionParticleEffect(xplosion) {
    ...
    handleFrameData(frameInfo, raw) {
        ...
        const ants = this.currentTempCellData.ants.map(arr => [...arr]);
        const richness = [...this.currentTempCellData.richness];
        const eventMapPerPlayer = [{}, {}];
        const antTotals = [
            sum(ants[0]),
            sum(ants[1])
        ];
        const consumedFrom = [new Set(), new Set()];
        // Handle explosions
        const shouldExplodeThisFrame = new Set();
        for (const event of dto.events) {
            eventMapPerPlayer[event.playerIdx][event.type] = (_a = eventMapPerPlayer[event.playerIdx][event.type]) !== null && _a !== void 0 ? _a : [];
        ...
        const foodEvents = this.globalData.players.map(p => { var _a; return computePathSegments((_a = eventMapPerPlayer[p.index][ev.FOOD]) !== null && _a !== void 0 ? _a : [], p.index, ev.FOOD); });
        const buildEvents = this.globalData.players.map(p => { var _a; return computePathSegments((_a = eventMapPerPlayer[p.index][ev.BUILD]) !== null && _a !== void 0 ? _a : [], p.index, ev.BUILD); });
        ...
        const frameData = Object.assign(Object.assign(Object.assign({}, dto), frameInfo), { syntheticEvents: [...foodEvents, ...buildEvents].filter(v => v != null), previous: null, buildAmount,
            ants,
            richness,
            antTotals,
            consumedFrom });
        frameData.previous = (_b = last(this.states)) !== null && _b !== void 0 ? _b : frameData;
        this.states.push(frameData);
        this.currentTempCellData = Object.assign({}, frameData);
        return frameData;
    }
}
